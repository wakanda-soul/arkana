...
/// Swaps SOL from user into the vault pool and opens a new 365-day ORE staking tranche.
/// All fees and rent are paid by the user.
pub fn process_swap_and_deposit_sol_tranche(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    let args = SwapAndDepositSolTranche::try_from_bytes(data)?;
    let equivalent_skr_amount = u64::from_le_bytes(args.equivalent_skr_amount);
    let sol_lamports = u64::from_le_bytes(args.sol_lamports);
    if equivalent_skr_amount == 0 {
        return Err(ArkanaVaultError::ZeroDeposit.into());
    }
    ...
    // Rate: 1 SKR (9 dec = 1_000_000_000) = 1 ORE (11 dec = 100_000_000_000)
    let ore_amount = equivalent_skr_amount
        .checked_mul(100)
        .ok_or(ArkanaVaultError::MathOverflow)?;

    // 1. Transfer SOL lamports if specified
    if sol_lamports > 0 {
...
pub fn process_admin_create_tranche(accounts: &[AccountInfo<'_>], data: &[u8]) -> ProgramResult {
    let args = AdminCreateTranche::try_from_bytes(data)?;
    let ore_amount = u64::from_le_bytes(args.ore_amount);
    if ore_amount == 0 {
        return Err(ArkanaVaultError::ZeroDeposit.into());
    }
    ...
    // 1. Initialize or load UserVault for target user
    let (user_vault_addr, _) = user_vault_pda(target_user_info.key);
    user_vault_info.has_address(&user_vault_addr)?;
    ...
    if user_vault_info.data_is_empty() {
        create_program_account::<UserVault>(
            ...
            signer_info,
            &arkana_ore_vault_api::ID,
            &[USER_VAULT_SEED, target_user_info.key.as_ref()],
        )?;
        let user_vault =
            user_vault_info.as_account_mut::<UserVault>(&arkana_ore_vault_api::ID)?;
        user_vault.owner = *target_user_info.key;
        user_vault.tranche_count = 0;
        user_vault.total_staked_ore = 0;
        user_vault.total_yield_claimed = 0;
    }
    ...
    // 2. Create the new Tranche PDA for target user
    let (tranche_addr, _) = tranche_pda(target_user_info.key, next_tranche_id);
    tranche_info.has_address(&tranche_addr)?;
    ...
    let tranche = tranche_info.as_account_mut::<Tranche>(&arkana_ore_vault_api::ID)?;
    tranche.owner = *target_user_info.key;
    tranche.tranche_id = next_tranche_id;
    tranche.is_matured = 0;
    tranche.deposited_amount = ore_amount;
    tranche.deposited_at = clock.unix_timestamp;
